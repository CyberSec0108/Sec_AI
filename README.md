...
* **Sec_AI Core**: `MIT License`
* RAG 참조용으로 내려받은 **공공기관 지침 문서(PDF)** 는 해당 기관의 이용 허락 규정(공공누리 등)을 따르며 무단 외부 재배포를 금지합니다.
* 오브젝트 스토리지는 AGPLv3 기반 AIStor(MinIO)를 사용하며, 엔터프라이즈 라이선스를 추가하면 Object Lock·KMS 기능으로 확장할 수 있는 구조입니다.

> **[라이선스 고지 및 아키텍처 구성 안내]**
> 본 프로젝트(Sec_AI)의 파이썬 핵심 애플리케이션 소스코드는 상업적 활용과 수정이 자유로운 **MIT 라이선스**를 채택하여 배포됩니다. 
> 시스템 운영에 필요한 ClamAV(GPL-2.0), MinIO(AGPL-3.0) 등 독립적이고 엄격한 라이선스 정책을 가진 핵심 인프라 소프트웨어들의 경우, 소스코드에 직접 결합(Linking)하지 않고 **개별 Docker 컨테이너 및 네트워크(API) 통신 방식으로 완벽히 분리 구성**했습니다. 이를 통해 파이썬 핵심 코드에 타 오픈소스의 라이선스(Copyleft)가 전염되거나 충돌하는 현상을 원천적으로 방지하고, 각 소프트웨어의 저작권 규정을 안전하게 준수하는 상용화 수준의 아키텍처를 구현했습니다.
