...
#### Windows PC — 동일 PC 방식 (원격은 작업 중)

수집 프로그램을 실행하면 브라우저가 열리고, 그 화면에서 점검을 시작합니다. **현재는 서버가 같은 PC에 있어야 합니다**(프로그램과 브라우저가 `127.0.0.1`로만 통신).

> 🚧 **원격 점검은 작업 중입니다.**
> Linux(아래)에 적용한 승인 방식을 Windows에도 확장하는 작업이 진행 중입니다. 승인·토큰·사이드카 계약은 이미 공통 모듈로 구현되어 있고, 남은 것은 **Windows 증적 패키지 빌더와 서버 업로드 경로**입니다. 완료 전까지 Windows는 위의 동일 PC 방식으로 사용하세요.
>
> 남은 구현 순서: ① Windows 패키지 빌더 → ② 패키지 JSON Schema·검증 규칙 → ③ 서버 업로드·판정 저장 → ④ 승인 핸드셰이크 배선(공통 모듈 재사용)
>
> 참고로 현재 Windows 실행 파일은 자체 서명(신뢰되지 않은 루트)이라, 빌드하지 않은 다른 PC에서 실행하면 SmartScreen 경고가 표시됩니다. 원격 배포 시에는 정식 코드 서명 인증서가 별도로 필요합니다.
...
2. 다운로드 화면에서 Linux 파일을 받고, **[이 서버로 결과 보내기]** 를 눌러 설정 파일(`*.secai-scan.json`)도 함께 받습니다.
